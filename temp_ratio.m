...
multi_alpha = [1 1.5  2 2.5  3];
me = 0.000548579909; % electron mass in amu
mi = 2.0; % Deuterium mass in amu
% T_ee = [20 40 50 70 100 120 150 200 250];    % Electron temp
% T_e = 200;    % Electron temp
% ratio_range = linspace(0,1,10);
% T_ii = ratio_range.*T_e;    % Ion temp
T_e = 10;    % Electron temp
T_ii = [1 10 20 30 50 60 70 80];
ratio_range = T_ii./T_e;
gamma =1;
...
for mm = 1:length(ratio_range)
    ...
    waitbar(mm/length(ratio_range),h,sprintf('%2.2f %% done',(100*mm/length(ratio_range))))
...
%         Cs = sqrt((kTe+(gamma*kTi))/amu);
    ...
    %     den = den_norm.*n_CS;
        ...
        %KE(k) = (1/(1.6E-19))*(0.5*amu*((Vx(length(a))*Vx(length(a)))+(Vy(length(a))*Vy(length(a)))+(Vz(length(a))*Vz(length(a)))));
        KE(k) = (1/(1.6E-19))* (0.5*amu*( (Vx(k,length(a)).^2)+ (Vy(k,length(a)).^2) + (Vz(k,length(a)).^2)));
        KE(k) = KE(k)./3;
        KE3d(mm,k) = KE(k);
    %     fprintf('den= %4.3f Vz= %4.3f Vy= %4.3f Vx= %4.3f Flux= %4.3f KE=%4.3f\n',den(length(a)),Vz(length(a)),Vy(length(a)),Vx(length(a)),Flux(k),KE(k));
...
figure(4)
surf(multi_alpha,ratio_range,Phys3d,'FaceColor','interp','EdgeColor','none','FaceLighting','gouraud');colormap jet;
% ylim([20 250]);
% ylim([20 700]);
view(-41,18);
xlabel('Angle(\alpha^{\circ})','rot',21 );
ylabel('Temperature ratio(T_i/T_e)','rot',-20);
% xlabel('Angle(\alpha^{\circ})');
% ylabel('Temperature ratio(T_i/T_e)');
title('Physical Sputtering Yield (Be)');
% camlight; lighting phong;


% figure(6)
% plot(T_ii,KE3d(:,1),'-o','LineWidth',1.5,...
%                     'MarkerEdgeColor','k',...
%                     'MarkerFaceColor','r',...
%                     'MarkerSize',5);
% %         ylim([0 80]);
